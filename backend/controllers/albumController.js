...
const obtenerAlbumes = async (req, res, next) => {
    try {
        const result = await albumService.listarAlbumes(req.query);
...
const obtenerAlbumPorId = async (req, res, next) => {
    try {
        const album = await albumService.obtenerPorId(req.params.id);
...
const crearAlbum = async (req, res, next) => {
    try {
        const album = await albumService.crearAlbum(req.body);
...
const eliminarAlbum = async (req, res, next) => {
    try {
        const info = await albumService.eliminarAlbum(req.params.id);
...
const buscarAlbumes = async (req, res, next) => {
    try {
        const result = await albumService.buscarAlbumes(req.query);
...
const obtenerAlbumesPorArtista = async (req, res, next) => {
    try {
        const albums = await albumService.albumsPorArtista(req.params.artista);
...
const obtenerAlbumesPorCategoria = async (req, res, next) => {
    try {
        const albums = await albumService.albumsPorCategoria(req.params.categoria);
...
const actualizarStock = async (req, res, next) => {
    try {
        ...
        const info = await albumService.actualizarStock(req.params.id, cantidad);
        if (!info) return fail(res, 'Álbum no encontrado', 404);
...
const obtenerEstadisticas = async (req, res, next) => {
    try {
        const data = await albumService.obtenerEstadisticas();
        return ok(res, { data });
    } catch (error) {
        next(error);
    }
